	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.shangjiaChatId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('shangjiaChat', this.shangjiaChatId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.shangjiaChatPhoto ? this.detail.shangjiaChatPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('shangjiaChatYuyue', {
									yonghuId: _this.user.id,
									shangjiaChatId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/shangjiaChatYuyue/list');
							}
						}
					});
				},
