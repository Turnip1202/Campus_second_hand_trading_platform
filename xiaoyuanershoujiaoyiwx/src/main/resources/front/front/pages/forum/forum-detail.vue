<template>
	<view>
        ...
        <view class="username">
			...
			<text v-if="forum.shangjiaId != null">
				 发布人：{{forum.shangjiaName}}（商家）
			</text>
			...
			<view class="uni-article__comment-box" v-for="comment in commentData" :key="comment.id" >
				<view class="article-meta">
					<view class="article-header-image">
                        ...
                        <image v-if="comment.shangjiaId != null" class="image" :src="baseUrl+comment.shangjiaPhoto" mode="widthFix"></image>
					...
					<view class="article-header-content">
						<view class="article-header-title">
							...
							<text v-if="comment.shangjiaId != null">{{comment.shangjiaName}}</text>
