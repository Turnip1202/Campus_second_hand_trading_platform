...
//    	ValidatorUtils.validateEntity(user);
        Wrapper<ShangjiaEntity> queryWrapper = new EntityWrapper<ShangjiaEntity>()
            ...
            .eq("shangjia_delete", 1)
            ;
        System.out.println(shangjia.getUsername());

        Wrapper<YonghuEntity> queryWrapperYtoS = new EntityWrapper<YonghuEntity>()
                .eq("username", shangjia.getUsername());
        YonghuEntity yonghuEntity = yonghuService.selectOne(queryWrapperYtoS);
        System.out.println(yonghuEntity);
        if(yonghuEntity == null)
            return R.error("账号未在小程序端注册,请注册账户后再来");
