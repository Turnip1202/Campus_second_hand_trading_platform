    ...
    */
    ...
    public R save(@RequestBody ShangpinEntity shangpin, HttpServletRequest request){
        ...
        System.out.println(role);
         String us=  String.valueOf(request.getSession().getAttribute("username"));
        System.out.println(us);
         boolean is_shangjia = shangjiaService.selectByName(us);
        if ("用户".equals(role))//判断是否是小程序用户
        if (!is_shangjia) {//查询是否注册为商家
            return R.error(511, "用户没有权限,请进行商家注册");
        };
