	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.shangpinCollectionId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('shangpinCollection', this.shangpinCollectionId);
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('shangpinCollectionYuyue', {
